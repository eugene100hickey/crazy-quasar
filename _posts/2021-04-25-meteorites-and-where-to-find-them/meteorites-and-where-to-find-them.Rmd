...
description: |
  Here we look at the record of meteorites, what they look like, where and how they were found. And we do it with lot of maps.
author:
  - name: Eugene
...
```{r libraries, echo=F, warning=F, message=F, cache=F}
...
font_add_google(name = "Handlee", family = "Handlee")
showtext_auto()

ggplot2::theme_set(ggplot2::theme_minimal())
theme_update(text = element_text(size = 28, family = "Handlee"))

cbPalette <- c("#111151", "#CCCCCC", "#4379ac","#FFFF00", 
...
meteors <- read_csv("../../data/Meteorite_Landings.csv")
...
meteors <- meteors %>% 
  mutate(year=lubridate::year(dmy_hms(year))) %>% 
...
```{r decade_plot, echo=F, warning=F, message=F, cache=F, preview=TRUE}
...
meteors %>% ggplot(aes(x=kg)) +
  ...
  xlim(0, 8) + 
  ylim(0, 200) +
  ggtitle("Histogram of Masses for all Meteorites") +
  xlab("Mass (kg)") +
  ylab("Frequency") +
  scale_y_log10() +
...
meteors %>% 
  ...
  geom_freqpoly(show.legend = T, bins=10) +  
  xlab("mass (kg)") +
  scale_y_log10() +
  xlim(0, 25) +
...
Not much to see here. North America is the place to go for larger meteorites, but note the plethora of small meteorites from Antarctica. In fact, by far the greatest number of meteorites come from Antarctica (`r meteors %>% filter(continent=="Antarctica") %>% summarise(n())` of them out of a total number of `r meteors %>% summarise(n())`), something that's hard to appreciate from the map above because they are frequently ascribed the same latitude, longitude and so their pointers sit on top of each other.
...
meteors %>% 
  ...
  geom_jitter(aes(), alpha=0.2, size=ifelse(meteors$kg > 2000 & meteors$year > 2000, 4, 0.5)) +  
  ...
  geom_segment(data=arrows1, aes(x=x, xend=xend, y=y, yend=yend), 
               col="black", size=0.5, 
               arrow=arrow(type="open", angle=10, length = unit(0.15, "inches"))) +
  annotate(geom="text", x=1999, y=1.5e4, label="Xifu", col="black", cex=6) + 
  annotate(geom="text", x=2007, y=4e4, label="Al Haggounia", col="black", cex=6) +
  annotate(geom="text", x=1979, y=140, label="Yamato", col="firebrick4", cex=6) + 
  annotate(geom="text", x=1988, y=140, label="Asuka", col="firebrick4", cex=6) +
  annotate(geom="text", x=1998, y=200, label="Queen\nAlexandra\nRange ", col="firebrick4", cex=6) +
  labs(x = "")
...
non_iron_meteors <- meteors %>% 
  filter(chond != "irons" & chond != "Stony_Iron" & chond != "achondrite")

res_aov <- aov(kg~chond, data = non_iron_meteors)

```

The four classes of chondrite seem to have similar mass distributions, something that can be investigated by an ANOVA test. The result is a p-value of `r signif(unlist(summary(res_aov))[9], 2)`. Looks like all the chondrites do indeed have similar masses. To have a closer look, a Tukey Test comes up with the following table

```{r tukey, echo=F, warning=F, message=F, cache=F} 
z <- tidy(TukeyHSD(res_aov))
z[,-c(1:2)] =sapply(z[,-c(1:2)], round, digits=2)
kable(z[,-1])
