...
Face++ needs registration and authorisation keys, the post from Theresa mentioned about discusses how to do this. I wasn't keen to have my key on github, so it's created here from a file outside the repo.
...
And now we can run our function. We have to manually code the character names, and I decided to recode the position so hat the origin was in the bottom left corner to make it synchronise better with the plot to come.
